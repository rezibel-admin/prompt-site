...
// --- DATA: PSYCHOLOGICAL PACKAGES WITH SLOGANS ---
const PRESET_PACKAGES = [
  { 
    id: 'pkg_core', 
    name: 'CORE', 
    slogan: 'مهندسی حضورِ استاندارد برای برندهایی که اشتباه در شروع را نمی‌پذیرند.',
    price: '25', 
    type: 'Core', 
    details: [
      ...
      "۵ فریم عکاسی تراز اول 4K (همراه با تجهیزات تخصصی نورپردازی و لنز ماکرو!): ثبت جزئیات محصول با وضوحِ میکروسکوپی.",
      ...
      "مدیریت تعامل و چرخه فروش: تبدیل ترافیک پیج به سرمایه با رعایت اصول متقاعدسازی حرفه‌ای.",
  ...
  { 
    id: 'pkg_fusion', 
    name: 'FUSION', 
    slogan: 'هم‌افزایی استراتژیک: نقطه تلاقی حاکمیتِ بصری و پردازش الگوریتمیک برای تسلطِ بی‌قید و شرط بر بازار.',
    price: '45', 
    type: 'Fusion', 
    details: [
      ...
      "۱۰ فریم عکاسی صنعتی/تبلیغاتی: ایجاد یک آرشیو بصریِ غنی و خیره‌کننده از محصولات و برند.",
      ...
      "پروتکل سئو بصری: مهندسی کپشن و بهینه‌سازی تصاویر برای تضمینِ حضور در ویترین جهانی (اکسپلور).",
      "مدیریت کامل ادمینی و فروش: واگذاری کامل چرخه جذب و تبدیل مشتری به تیم متخصص."
    ]
  },
  { 
    id: 'pkg_quantum', 
    name: 'QUANTUM', 
    slogan: 'حاکمیت ابدی در فضای دیجیتال؛ پکیجی برای رهبرانی که به چیزی جز «نفر اول بودن» قانع نیستند.',
    price: '85', 
    type: 'Quantum', 
    details: [
      "۸ اپیزود تولید محتوای سینمایی: شامل ۲ محتوای ویژه با تصویربرداری هوایی FPV برای خلق ویدیوهایی در سطح جهانی.",
      "۲۰ فریم عکاسی اشرافی: نمایش شکوه بیزنس با دقتی که اعتمادِ سخت‌گیرترین مشتریان را جلب می‌کند.",
      "۱۲ شاهکار سنتز هوشمند (Total AI Domination): ۴ اپیزود انیمیشن سینمایی + ۸ ویدئوی هدفمند؛ پوشش کامل نیازهای بصری سالانه.",
      "مهندسی بیزنس‌پلان اختصاصی: تدوین استراتژی‌های کلان برای تبدیلِ «بیننده» به «سرمایه پایدار».",
      "خلق سفیر مجازی برند (Virtual Influencer): طراحی یک شخصیت دیجیتال اختصاصی که به نمادِ فناناپذیر برند شما تبدیل می‌شود.",
      "حاکمیت بر اکسپلور: مهندسی محتوا با هدف ورود حداکثری به ویترین جهانی."
    ]
  },
  { 
    id: 'pkg_tactical', 
    name: 'TACTICAL', 
    slogan: 'ابزارهای هدفمند برای تقویت جبهه‌های خاص کسب‌وکار.',
    price: 'Variable', 
    type: 'Tactical', 
    details: [
      ...
      "تخت پادشاهی دیجیتال (Web Architecture): طراحی وب‌سایتی که قلبِ تپنده‌ی امپراتوری شماست."
    ]
  }
];
  ...
  return (
    <div className="min-h-screen bg-[#010101] text-white overflow-x-hidden font-sans selection:bg-[#40E0D0]/30 pb-20 relative">
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        /* Animated Silver Liquid Halos */
        .halo-bg { position: fixed; top: 0; left: 0; width: 100%; height: 100%; z-index: -1; pointer-events: none; }
        .halo-1 { position: absolute; top: -10%; left: -10%; width: 70%; height: 70%; background: radial-gradient(circle, rgba(200,200,200,0.06) 0%, transparent 70%); animation: smoke 20s infinite alternate; }
        .halo-2 { position: absolute; bottom: -10%; right: -10%; width: 60%; height: 60%; background: radial-gradient(circle, rgba(64,224,208,0.04) 0%, transparent 70%); animation: smoke 25s infinite alternate-reverse; }
        @keyframes smoke { from { transform: translate(0,0) scale(1); } to { transform: translate(15%, 15%) scale(1.15) rotate(5deg); } }
        ...
        @keyframes pulse-n { 0%, 100% { opacity: 0.3; transform: scale(1); } 50% { opacity: 1; transform: scale(1.15); } }
        
        .gold-glow-badge { background: rgba(212, 175, 55, 0.1); border: 1px solid rgba(212, 175, 55, 0.4); box-shadow: 0 0 20px rgba(212, 175, 55, 0.15); }
      `}} />
      ...
      <div className={`transition-opacity duration-1000 ${entered ? 'opacity-100' : 'opacity-0'}`}>
        ...
        <nav className="p-8 md:p-10 flex justify-between items-center fixed top-0 w-full z-[100] backdrop-blur-3xl bg-black/30 border-b border-white/5">
           <div className="flex gap-4">
             <button onClick={() => setIsMuted(!isMuted)} className="p-3 glass-luxury rounded-full shadow-2xl transition-all">
               {isMuted ? <VolumeX className="text-red-500" /> : <Volume2 className="text-[#40E0D0]" />}
             </button>
             <button onClick={() => setShowLoginModal(true)} className="p-3 glass-luxury rounded-full hover:text-[#40E0D0] transition-colors shadow-2xl">
               <Lock size={20}/>
             </button>
           </div>
           <div className="text-right">
             <div className="text-xl md:text-2xl font-black tracking-tighter italic">PROMPT</div>
             <div className="text-[7px] tracking-[0.4em] text-[#40E0D0] uppercase italic font-bold">Sovereign Authority</div>
           </div>
        </nav>
        ...
        <main className="max-w-[1900px] mx-auto px-6 py-40">
          ...
          <section className="mb-80 relative group">
            <div className="flex justify-between items-end mb-16 border-b border-white/5 pb-10">
               ...
               <p className="text-[#40E0D0] tracking-[0.5em] text-[10px] uppercase font-bold italic">Neural Cinematic Repository</p>
            </div>
            ...
            >
              {portfolio.map((item) => (
                <div key={item.id} className="min-w-[80vw] md:min-w-[500px] aspect-[9/16] md:aspect-[3/4] rounded-[4.5rem] overflow-hidden border border-white/5 glass-luxury snap-center transition-all duration-700 hover:scale-[1.02] hover:border-[#40E0D0]/40 relative group/card shadow-[0_0_80px_rgba(0,0,0,0.5)]">
                  <div className="absolute inset-0 cursor-pointer" onClick={() => handleVideoOpen(item)}>
                    ...
                    <div className="absolute inset-0 flex flex-col justify-end p-12 bg-gradient-to-t from-black/95 via-transparent">
                      <div className="w-16 h-16 rounded-full border border-[#40E0D0]/40 flex items-center justify-center text-[#40E0D0] mb-6 backdrop-blur-2xl group-hover/card:bg-[#40E0D0] group-hover/card:text-black transition-all">
            ...
            <div className="md:hidden flex flex-col items-center mt-8 gap-4 opacity-50">
               <div className="flex items-center gap-10">
                 <button onClick={() => scroll('left')} className="p-4 glass-luxury rounded-full text-[#40E0D0] pulse-neon"><ChevronLeft size={24}/></button>
                 <span className="text-[10px] tracking-[0.4em] uppercase font-bold text-zinc-500">Explore Vault</span>
                 <button onClick={() => scroll('right')} className="p-4 glass-luxury rounded-full text-[#40E0D0] pulse-neon"><ChevronRight size={24}/></button>
               </div>
            </div>
          </section>

          {/* --- PROPOSALS (SLOGAN SEPARATED) --- */}
          <section className="mb-80 relative">
             <div className="text-center mb-32">
               ...
               <p className="text-zinc-500 tracking-[0.5em] uppercase text-[9px] font-black italic">Strategic Alliance Packages</p>
             </div>
             <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-12">
                {PRESET_PACKAGES.map(pkg => (
                  <div key={pkg.id} onClick={() => setPreviewPackage(pkg)} className={`group/pkg p-12 rounded-[4.5rem] min-h-[600px] flex flex-col justify-between cursor-pointer border border-white/5 glass-luxury hover:bg-white/5 transition-all duration-700 ${pkg.type === 'Fusion' ? 'border-[#40E0D0]/40 scale-105 z-10 bg-[#40E0D0]/5 shadow-[0_0_80px_rgba(64,224,208,0.05)]' : ''} ${pkg.type === 'Quantum' ? 'border-[#D4AF37]/40 shadow-[0_0_80px_rgba(212,175,55,0.05)]' : ''}`}>
                      <div>
                        ...
                        <h3 className="text-5xl font-black mb-2 tracking-tighter uppercase italic">{pkg.display_title}</h3>
                        <p className="text-[11px] text-[#40E0D0] mb-6 font-[Vazirmatn] leading-relaxed opacity-80 font-bold">{pkg.slogan}</p>
                      ...
                      <div className="mt-12 flex items-center gap-4 text-[11px] font-black uppercase tracking-[0.3em] opacity-30 group-hover/pkg:opacity-100 transition-all">Protocol Specs <Plus size={16} className="text-[#40E0D0]"/></div>
                  </div>
                ))}
             </div>
          </section>

          {/* --- NEURAL GRID (QUOTA + GOLD FREE BADGE) --- */}
          <section className="mb-64 relative">
            <div className="text-center mb-32">
               <div className="inline-flex items-center gap-3 mb-6 px-6 py-3 rounded-full bg-[#40E0D0]/10 text-[#40E0D0] border border-[#40E0D0]/30 animate-pulse">
                 <Bot size={20}/> <span className="text-xs font-black tracking-widest uppercase">Neural Protocol Active</span>
               </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
              {NEURAL_MODULES.map((module) => (
                <div key={module.id} className="p-10 rounded-[4rem] border border-white/5 glass-luxury hover:border-[#40E0D0]/60 transition-all duration-700 flex flex-col h-[600px] relative overflow-hidden group shadow-3xl">
                  {/* Luxury FREE Badge */}
                  <div className="absolute top-10 right-10 px-5 py-2 rounded-full gold-glow-badge text-[#D4AF37] text-[10px] font-black tracking-widest italic">FREE ACCESS</div>
                  
                  <div className="mt-12 mb-8 text-[#40E0D0] bg-white/5 p-6 rounded-[2.2rem] w-fit group-hover:scale-110 transition-transform duration-700 shadow-xl">{module.icon}</div>
                  <h3 className="text-2xl font-black text-white uppercase italic tracking-tight">{module.title}</h3>
                  <p className="text-[13px] text-[#40E0D0] mb-5 font-[Vazirmatn] font-bold italic tracking-tight">{module.fa_title}</p>
                  <p className="text-xs text-zinc-400 leading-loose mb-12 font-[Vazirmatn]">{module.desc}</p>
                  
                  <div className="mt-auto flex flex-col gap-6">
                    <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-widest text-zinc-600 italic">
                      <span>Daily Recharge Status</span>
                      <span className="text-[#40E0D0]">{quotas[module.id]}/{module.limit} PASSED</span>
                    </div>
        ...
        <footer className="mt-64 border-t border-white/5 bg-[#000] py-40 text-center md:text-right relative">
            <div className="max-w-[1900px] mx-auto px-10 flex flex-col md:flex-row justify-between items-center gap-24">
               <div className="p-10 rounded-[4.5rem] glass-luxury border border-[#40E0D0]/20 group transition-all duration-1000 hover:border-[#40E0D0] shadow-3xl">
                  <img src={`https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${window.location.href}&color=40E0D0&bgcolor=000&margin=2`} className="w-28 h-28 rounded-3xl opacity-40 group-hover:opacity-100 transition-opacity" alt="Sovereign QR" />
               </div>
               <div>
                  ...
                  <p className="text-[9px] text-zinc-800 tracking-[0.8em] uppercase mt-24 font-bold opacity-30 italic">© 2026 Sovereign Authority | Visual Hegemony Reserved</p>
               </div>
            </div>
        </footer>
      </div>

      {/* --- MODALS (ENFORCED PROTECTION) --- */}
      {activeVideo && (
      ...
      {previewPackage && (
        <div className="fixed inset-0 z-[5000] bg-black/96 flex items-center justify-center p-6 backdrop-blur-3xl" onClick={() => setPreviewPackage(null)}>
           <div className="max-w-5xl w-full p-16 glass-luxury rounded-[5.5rem] text-right border border-[#40E0D0]/20 max-h-[88vh] overflow-y-auto shadow-3xl relative" onClick={e=>e.stopPropagation()}>
             <h2 className="text-5xl md:text-8xl font-black mb-4 text-white tracking-tighter italic uppercase">{previewPackage.display_title}</h2>
             ...
             <div className="space-y-10" dir="rtl">
               {previewPackage.details.map((d,i) => (
                 <div key={i} className="flex items-start gap-6 text-xl md:text-2xl text-zinc-300 border-b border-white/5 pb-10 hover:text-[#40E0D0] transition-colors duration-500">
                   <CheckCircle2 className="text-[#40E0D0] mt-1.5 shrink-0" size={30}/> 
                   <div className="leading-relaxed drop-shadow-md">{formatPersianText(d)}</div>
                 </div>
               ))}
             </div>
           </div>
